  ...
  <div class="container_page">
    <div class="content-area">
      <el-row class="main">
        ...
        <el-col :span="24">
          <el-row class="sparkboxes">
            <el-col :span="12" style="padding: 0 4rem 0 0;">
              <IndicatorLines />
            </el-col>
            <el-col :span="10" style="padding: 0 4rem 0 0;">
...
const obterSparks = async (inicio: string, fim: string) => {
  ...
  sparks.value = {
    total: {
      value: result?.result?.total?.value || 0,
      values: result?.result?.total?.values || []
    },
    balanco: {
      value: result?.result?.balanco?.value || 0,
      values: result?.result?.balanco?.values || []
    },
    pendente: {
      value: result?.result?.pendente?.value || 0,
      values: result?.result?.pendente?.values || []
    },
    pago: {
      value: result?.result?.pago?.value || 0,
      values: result?.result?.pago?.values || []
    }
  };
}
